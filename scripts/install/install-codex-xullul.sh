...
RELEASE="${CODEX_XULLUL_RELEASE:-latest}"
REPO="${CODEX_XULLUL_REPO:-xullul/codex}"
COMMAND_NAME="${CODEX_XULLUL_COMMAND:-codex-xullul}"

BIN_DIR="${CODEX_XULLUL_INSTALL_DIR:-${CODEX_INSTALL_DIR:-$HOME/.local/bin}}"
BIN_PATH="$BIN_DIR/$COMMAND_NAME"
CODEX_HOME_DIR="${CODEX_XULLUL_HOME:-${CODEX_HOME:-$HOME/.codex}}"
STANDALONE_ROOT="$CODEX_HOME_DIR/packages/$COMMAND_NAME-standalone"
...
usage() {
  cat <<EOF
Usage: install-codex-xullul.sh [--release VERSION] [--repo OWNER/REPO] [--command NAME]

Environment:
  CODEX_XULLUL_REPO         GitHub repo to install from. Defaults to xullul/codex.
  CODEX_XULLUL_RELEASE      Release version or tag. Defaults to latest.
  CODEX_XULLUL_COMMAND      Command name to expose. Defaults to codex-xullul.
  CODEX_XULLUL_INSTALL_DIR  Directory for the command shim. Defaults to CODEX_INSTALL_DIR or ~/.local/bin.
  CODEX_XULLUL_HOME         Install state directory. Defaults to CODEX_HOME or ~/.codex.
EOF
}
...
download_file() {
  ...
  echo "curl or wget is required to install $COMMAND_NAME." >&2
  exit 1
}
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the $COMMAND_NAME download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded $COMMAND_NAME archive checksum did not match release metadata." >&2
...
resolve_version() {
  ...
  if [ -z "$resolved" ]; then
    echo "Failed to resolve the latest $COMMAND_NAME release version from $REPO." >&2
    exit 1
  fi
...
add_to_path() {
  ...
  begin_marker="# >>> $COMMAND_NAME installer >>>"
  end_marker="# <<< $COMMAND_NAME installer <<<"
...
maybe_launch_now() {
  if prompt_yes_no "Start $COMMAND_NAME now?"; then
    step "Launching $COMMAND_NAME"
    "$BIN_PATH"
  fi
}
...
case "$(uname -s)" in
  ...
  Linux)
    ...
    ;;
  *)
    echo "install-codex-xullul.sh supports macOS and Linux. Use install-codex-xullul.ps1 on Windows." >&2
    exit 1
    ;;
esac
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating $COMMAND_NAME from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating $COMMAND_NAME"
else
  step "Installing $COMMAND_NAME"
fi
step "GitHub repo: $REPO"
