...
impl App {
    ...
    pub(super) fn background_agent_activity_label(&self) -> Option<String> {
        ...
        let first_label = self.thread_label(*first_thread_id);
        let first = first_activity.label();
        let running_count = entries
            .iter()
            .filter(|(_, activity)| activity.is_active())
            .count();
        let more_running = running_count.saturating_sub(usize::from(first_activity.is_active()));

        let detail = if more_running == 0 {
            format!("{first_label} {first}")
        } else {
            format!("{first_label} {first}; {more_running} running")
        };
        Some(format!("Agents: {detail}"))
    }
