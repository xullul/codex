...
fn status_line_right_indicator(
    ...
    let mode_indicator = if show_cycle_hint {
        mode_indicator_line(collaboration_mode_indicator, show_cycle_hint)
    } else {
        compact_mode_indicator_line(collaboration_mode_indicator)
    };

    mode_indicator.or_else(|| goal_status_indicator_line(goal_status_indicator))
}
