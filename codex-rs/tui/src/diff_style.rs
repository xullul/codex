...
// that remain obvious across syntax themes.
...
pub(crate) fn style_syntax_for_diff(
    ...
    if matches!(kind, DiffLineType::Delete) {
...
mod tests {
    ...
    fn background_style_snapshot(
        ...
        let add_sign = style_sign_add(theme, color_level, backgrounds);
        let del_sign = style_sign_del(theme, color_level, backgrounds);
    ...
    fn ansi16_uses_foreground_only_diff_styles() {
        ...
        assert_eq!(
            style_sign_add(DiffTheme::Dark, DiffColorLevel::Ansi16, backgrounds),
            Style::default()
                .fg(Color::Green)
                .add_modifier(Modifier::BOLD)
        );
        assert_eq!(
            style_sign_del(DiffTheme::Dark, DiffColorLevel::Ansi16, backgrounds),
    ...
    #[test]
    fn syntax_span_background_is_replaced_with_semantic_diff_background() {
        let backgrounds = fallback_diff_backgrounds(DiffTheme::Dark, DiffColorLevel::TrueColor);
    ...
    #[test]
    fn delete_syntax_span_keeps_foreground_and_adds_dim() {
        let backgrounds = fallback_diff_backgrounds(DiffTheme::Dark, DiffColorLevel::TrueColor);
