...
const TRANSCRIPT_HINT: &str = "Ctrl+T for full output";
...
impl ExecCell {
    ...
    fn command_display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let action_summary = summarize_call(call, &cmd_display);
        let mut lines: Vec<Line<'static>> = if let Some(summary) = &action_summary {
            let mut header_line = Line::from(vec![
                bullet.clone(),
                " ".into(),
                summary.verb(self.is_active()).cyan().bold(),
            ]);
            ...
            if let Some(detail) = &summary.detail {
                ...
                let detail_line = Line::from(detail.clone().dim());
                ...
                push_owned_lines(
                    &adaptive_wrap_line(&detail_line, first_opts),
                    &mut first_wrapped,
                );
        ...
        if let Some(output) = call.output.as_ref() {
            let suppress_success_output = output.exit_code == 0
                && action_summary
                    .as_ref()
                    .is_some_and(|summary| summary.suppress_success_output);
            ...
            } else if !suppress_success_output {
    ...
    /// Builds an output ellipsis line (`… +N lines (Ctrl+T for full output)`)
...
mod tests {
    ...
    fn truncate_lines_middle_keeps_omitted_count_in_line_units() {
        ...
        assert!(
            rendered
                .iter()
                .any(|line| line.contains("… +6 lines (Ctrl+T for full output)")),
    ...
    fn output_lines_ellipsis_includes_transcript_hint() {
        ...
        assert!(
            rendered
                .iter()
                .any(|line| line.contains("… +3 lines (Ctrl+T for full output)")),
