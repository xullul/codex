...
//! background lightness.  Rich color terminals get strong full-line semantic
//! green/red backgrounds with stronger line-number gutters; ANSI-16 terminals
//! keep foreground-only styling for readability.
//!
...
//! `diff.inserted` / `diff.deleted`) scopes, those colors are used only when
//! they are at least as strong as the semantic diff palette.  Weak theme
//! backgrounds fall back to the semantic palette so edit blocks stay obvious.
//!
...
use crate::diff_style::DiffColorLevel;
use crate::diff_style::DiffRenderStyleContext;
use crate::diff_style::DiffTheme;
use crate::diff_style::ResolvedDiffBackgrounds;
pub(crate) use crate::diff_style::current_diff_render_style_context;
use crate::diff_style::style_add;
use crate::diff_style::style_context;
use crate::diff_style::style_del;
use crate::diff_style::style_gutter_for;
use crate::diff_style::style_line_bg_for;
use crate::diff_style::style_sign_add;
use crate::diff_style::style_sign_del;
use crate::diff_style::style_syntax_for_diff;
...
fn push_wrapped_diff_line_inner_with_theme_and_color_level(
    ...
    let (sign_char, sign_style, content_style) = match kind {
        DiffLineType::Insert => (
            '+',
            style_sign_add(diff_backgrounds),
            style_add(theme, color_level, diff_backgrounds),
        ),
        DiffLineType::Delete => (
            '-',
            style_sign_del(diff_backgrounds),
    ...
    if let Some(syn_spans) = syntax_spans {
        ...
        let styled: Vec<RtSpan<'static>> = syn_spans
            .iter()
            .map(|sp| {
                let style = style_syntax_for_diff(kind, sp.style, line_bg);
...
mod tests {
    use super::*;
    use crate::diff_style::fallback_diff_backgrounds;
    ...
    fn light_theme_wrapped_lines_keep_number_gutter_contrast() {
        ...
        assert_eq!(lines[1].spans[0].style, gutter_style);
        assert_eq!(lines[0].style, line_style);
        assert_eq!(lines[1].style, line_style);
    }
    ...
    #[test]
    fn tiny_summary_width_wraps_huge_tokens_before_ratatui_reflow() {
        let mut changes: HashMap<PathBuf, FileChange> = HashMap::new();
        changes.insert(
            PathBuf::from("huge-token.txt"),
            FileChange::Add {
                content: format!("{}\n", "x".repeat(70_000)),
            },
        );

        let lines = create_diff_summary(&changes, &PathBuf::from("/"), /*wrap_cols*/ 2);
        let max_span_width = lines
            .iter()
        ...
        assert!(
            max_span_width < 100,
            "tiny summary widths must pre-wrap huge tokens before ratatui reflow: {max_span_width}"
        );
    }
