...
- `commandExecution` — `{id, command, cwd, status, commandActions, aggregatedOutput?, exitCode?, durationMs?}` for sandboxed commands; `status` is `inProgress`, `completed`, `failed`, or `declined`.
  `commandActions` is a best-effort display model. Known variants are `read`, `listFiles`, `search`, `action`, and `unknown`. The `action` variant is used when a command is better summarized by intent than by a file target, and includes `{command, kind, detail}`. `kind` is one of `inspect`, `edit`, `test`, `build`, `lint`, `git`, `wait`, or `run`; clients should keep handling `unknown` and tolerate newer variants for forward compatibility.
